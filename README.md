...
├── lines.json          # 384条可追溯的中文爻辞
├── scene_mapping.json  # 场景到卦象的映射
├── evaluation_cases.json # 30个现实决策回归案例
├── SOURCES.md          # 经典文本的数据来源与许可
└── yao_attributes.json # 爻位属性定义
```

爻辞原文来自维基文库公有领域《周易》，每条记录保存具体来源 URL；可用
`python scripts/import_wikisource_lines.py` 重新导入并校验。爻名由卦象
`binary_code` 和爻位统一计算，避免阴阳爻名与互卦计算产生两套事实。

### 如何优化数据
1. **扩充现实评测**：为`evaluation_cases.json`添加更多真实决策与人工评审标准
2. **完善白话解读**：在`lines.json`中补充经人工审核的`plain_explanation`
3. **接入变卦逻辑**：将`bian_gua.py`中的变卦规则更完整地接入报告流程

### 如何改进程序
1. **Agent个性化**：CLI 与 Web 已按角色读取独立模型配置，可继续增加成本与耗时统计
...
| **爻辞数据** | ✅ 已完成 | 384条中文原文、来源 URL 与阴阳爻名一致性校验 |
| **互卦分析** | ✅ 已接入 | 统一使用卦象二进制计算，CLI 与 Web 共用 |
| **决策报告 V2** | ✅ 已接入 | 决策倾向、推进/停止条件、缺失信息与复评触发点 |

### 🚀 高级功能规划

| 功能 | 状态 | 说明 |
|------|------|------|
| **用户结果反馈** | ✅ Web版已接入 | 记录最终决定、实际结果与1-5分帮助度 |
| **历史记录** | ✅ Web版已接入 | 推演完成后自动保存，支持查看、复盘和删除 |
| **导出功能** | ✅ Markdown | Web版支持复制和Markdown下载；PDF仍待开发 |

### 📊 数据优化方向

| 优化点 | 状态 | 说明 |
|--------|------|------|
| **场景映射扩展** | 🚧 持续完善 | 已加入细分场景并用30个现实案例做本地回归 |
